...
async function initGame() {
  ...
  const resizeObserver = new ResizeObserver(() => {
    ...
    if (container && game.scale) {
      ...
      if (w > 0 && h > 0) {
        game.scale.resize(LAYOUT.game.width, LAYOUT.game.height);
        game.scale.setGameSize(LAYOUT.game.width, LAYOUT.game.height);
      }
    }
  });
  const container = document.getElementById('game-container');
...
function preload() {
  ...
  this.load.image('office_bg', './office_bg_small' + (supportsWebP ? '.webp' : '.png') + '?v=1774404499');
  this.load.spritesheet('star_idle', './star-idle-v5' + getExt('star-idle-spritesheet.png'), { frameWidth: 128, frameHeight: 128 });
  this.load.spritesheet('star_researching', './star-idle-v5' + getExt('star-researching-spritesheet.png'), { frameWidth: 128, frameHeight: 105 });

  this.load.image('sofa_idle', './sofa-idle-v3' + getExt('sofa-idle.png'));
  this.load.spritesheet('sofa_busy', './sofa-idle-v3' + getExt('sofa-busy-spritesheet.png'), { frameWidth: 256, frameHeight: 256 });
  ...
  this.load.spritesheet('coffee_machine', './coffee-machine-v3-grid' + getExt('coffee-machine-spritesheet.png'), { frameWidth: 230, frameHeight: 230 });
