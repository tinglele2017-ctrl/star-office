(function(){
  ...
  window.fetch=function(url,opts){
    ...
    if(u.indexOf('/set_state')>=0 && opts && opts.body){
      try{var d=JSON.parse(opts.body);if(d.state){_state=d.state;_detail=d.detail||d.state;}}catch(e){}
      return Promise.resolve(new Response(JSON.stringify({ok:true}),{headers:{'Content-Type':'application/json'}}));
    }
    if(u.indexOf('/status')>=0)return Promise.resolve(new Response(JSON.stringify({status:'online',agents:AGENTS.length,state:_state,detail:_detail}),{headers:{'Content-Type':'application/json'}}));
